...
void print_hdr_tcp(uint8_t *buf) {
    ...
    fprintf(stderr, "\tacknowledgement: %u\n", ntohl(tcp_hdr->tcp_ack));
    fprintf(stderr, "\tdata offset: %d\n", tcp_hdr->tcp_off);
    
    if (tcp_hdr->tcp_ctl & TCP_URG)
        fprintf(stderr, "\tcontrol bit: URG\n");
    if (tcp_hdr->tcp_ctl & TCP_ACK)
        fprintf(stderr, "\tcontrol bit: ACK\n");
    if (tcp_hdr->tcp_ctl & TCP_PSH)
        fprintf(stderr, "\tcontrol bit: PSH\n");
    if (tcp_hdr->tcp_ctl & TCP_RST)
        fprintf(stderr, "\tcontrol bit: RST\n");
    if (tcp_hdr->tcp_ctl & TCP_SYN)
        fprintf(stderr, "\tcontrol bit: SYN\n");
    if (tcp_hdr->tcp_ctl & TCP_FIN)
        fprintf(stderr, "\tcontrol bit: FIN\n");
